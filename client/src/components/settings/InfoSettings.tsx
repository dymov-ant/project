...
import { IProfileToUpdate } from "../../types/profileTypes"
...
export const InfoSettings: FC = () => {
  ...
  const userId = useSelector((state: GlobalState) => state.authReducer.currentUser?.profile.id)
  // const currentUser = useSelector((state: GlobalState) => state.authReducer.currentUser?.profile)
  const currentUser = useSelector((state: GlobalState) => state.profileReducer.profile)
  const initialValues: IProfileToUpdate = {
    ...
    <Paper elevation={2} className={classes.paper}>
      ...
      <Formik
        ...
        onSubmit={(values, {setSubmitting}) => {
          setSubmitting(true)
          userId && dispatch(updateProfile(userId, values))
          setSubmitting(false)
        }}
      >
